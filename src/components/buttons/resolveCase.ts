...
import { resolveCase as resolveCaseService } from '../../services/moderationService';
import {
 ...
 */
export async function handleResolveCase(interaction: ButtonInteraction): Promise<void> {
  ...
  try {
    ...
    if (!validation.isValid) {
      ...
      await interaction.reply(
        createButtonResponse(
          'error',
          'Permission Denied',
          validation.errorMessage || 'You do not have permission to resolve cases.'
        )
      );
      return;
    }
    ...
    const result = await resolveCaseService(
      caseId,
      interaction.user.id,
      'Case resolved by moderator via Discord interface'
    );
    ...
    await updateEmbedWithResolvedStatus(interaction, 'Alert Dismissed', 0x00ff00); // Green
    ...
    try {
      await sendCaseRecordToChannel(
        caseId,
        playerId || 'unknown',
        result.data, // Pass the full case data
        'resolve',
        'Case resolved by moderator',
        interaction.user.id
      );
    } catch (recordError) {
...
If new evidence emerges, a new case can be created.`;
    ...
    const errorResponse = createButtonResponse(
      'error',
      'Resolution Failed',
      'An error occurred while resolving the case. The case may still be open. Please try again or contact an administrator if the issue persists.',
      true
    );

    if (error instanceof Error && error.message.includes('not found')) {
      errorResponse.embeds[0].setDescription(
        'The specified case could not be found. It may have already been resolved or closed by another moderator.'
      );
    } else if (error instanceof Error && error.message.includes('already closed')) {
      errorResponse.embeds[0].setDescription(
        'This case has already been resolved or closed.'
      );
    }
 ...
 */
export const resolveCaseButton = new ButtonBuilder()
