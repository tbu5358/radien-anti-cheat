...
interface EnvVarConfig {
  ...
  sensitive?: boolean;
  mockDefault?: string;
  requiredInMock?: boolean;
  skipValidationInMock?: boolean;
}

interface EnvVarResult {
  value: string;
  usedMockDefault: boolean;
}

/**
 * Validate environment variable
 */
function validateEnvVar(config: EnvVarConfig, options: { isMockMode: boolean }): EnvVarResult {
...
function logEnvVarStatus(
  config: EnvVarConfig,
  value: string,
  options: { usedMockDefault?: boolean } = {}
): void {
 ...
 */
function createEnvironmentConfig(): Record<string, string> {
...
export const envConfig = {
  ...
  discord: {
    ...
    guildId: environment.DISCORD_GUILD_ID || null,
  },

  // Channel Configuration
  channels: {
    antiCheatPings: environment.ANTI_CHEAT_PINGS_CHANNEL,
    moderationLogs: environment.MODERATION_LOGS_CHANNEL,
    caseRecords: environment.CASE_RECORDS_CHANNEL,
    banReview: environment.BAN_REVIEW_CHANNEL,
  },

  // Backend API Configuration
  api: {
    baseUrl: environment.BACKEND_API_URL,
    key: environment.BACKEND_API_KEY,
  },
  ...
  app: {
    ...
    isDevelopment: (environment.NODE_ENV || 'development') === 'development',
    isProduction: (environment.NODE_ENV || 'development') === 'production',
    mockMode: isMockMode,
  },
};
...
export function checkEnvironmentHealth(): {
  ...
  for (const envConfig of envConfigs) {
    if (envConfig.required && !environment[envConfig.name]) {
