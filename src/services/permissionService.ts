...
export async function checkPermission(
  ...
  try {
    ...
    if (!permission) {
      throw new ValidationError('permission', permission, 'Permission is required');
    }
    ...
    logger.error(`Permission check failed for user`, { userId, error: error instanceof Error ? error.message : String(error) }, error instanceof Error ? error : undefined);
    throw error;
  }
}
...
export async function checkAnyPermission(
  ...
  try {
    ...
    if (!Array.isArray(permissions) || permissions.length === 0) {
      throw new ValidationError('permissions', permissions, 'At least one permission is required');
    }
  ...
  try {
    ...
    try {
      const response = await apiClient.get<
        ApiResponse<UserPermissionContext>
 ...
 */
async function getPermissionContextFromRoles(
  userId: string,
  guildId?: string
): Promise<UserPermissionContext> {
...
export async function grantPermissions(
  ...
  granterId: string,
  reason: string
): Promise<ApiResponse<{ granted: Permission[] }>> {
  try {
    ...
    if (result.success && result.data?.granted.length) {
      await createAuditLog({
        eventType: AuditEventType.PERMISSIONS_UPDATED,
        severity: AuditSeverity.WARNING,
        userId: granterId,
        targetId: userId,
        targetType: 'user',
...
export async function revokePermissions(
  ...
  revokerId: string,
  reason: string
): Promise<ApiResponse<{ revoked: Permission[] }>> {
  try {
 ...
 */
export async function updateRoleConfiguration(
  roleConfig: RolePermissions,
  updaterId: string
): Promise<ApiResponse<{ updated: boolean }>> {
  try {
    if (!updaterId?.trim()) {
      throw new ValidationError('updaterId', updaterId, 'Updater ID is required');
    }
 ...
 */
function validatePermissionOperation(
  userId: string,
  permissions: Permission[],
  operatorId: string
): void {
  ...
  if (!operatorId?.trim()) {
    throw new ValidationError('operatorId', operatorId, 'Operator ID is required');
  }

  // Validate each permission
  for (const permission of permissions) {
    if (!Object.values(Permission).includes(permission)) {
      throw new ValidationError('permissions', permissions, `Invalid permission: ${permission}`);
    }
  }
}
 ...
 */
function getRequiredLevelForPermission(permission: Permission): PermissionLevel {
  for (const [level, permissions] of Object.entries(PERMISSION_LEVEL_PERMISSIONS)) {
    if (permissions.includes(permission)) {
      return parseInt(level) as PermissionLevel;
    }
  }
  return PermissionLevel.ADMINISTRATOR; // Default to highest level for unknown permissions
}
 ...
 */
export async function canPerformModerationAction(
  userId: string,
  action: 'FLAG' | 'REQUEST_EVIDENCE' | 'SUBMIT_BAN' | 'APPROVE_BAN' | 'RESOLVE',
  guildId?: string
): Promise<boolean> {
  const permissionMap = {
    FLAG: Permission.FLAG_PLAYER,
    REQUEST_EVIDENCE: Permission.REQUEST_EVIDENCE,
    SUBMIT_BAN: Permission.SUBMIT_BAN_REVIEW,
    APPROVE_BAN: Permission.APPROVE_BAN,
    RESOLVE: Permission.RESOLVE_CASE,
  };
