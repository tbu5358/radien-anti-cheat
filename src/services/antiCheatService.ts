 ...
 */
export async function submitAntiCheatEvent(
  event: AntiCheatEvent
): Promise<ApiResponse<{ eventId: string; caseCreated: boolean }>> {
  try {
 ...
 */
export async function getAntiCheatEvent(
  eventId: string
): Promise<ApiResponse<AntiCheatEvent>> {
  try {
    if (!eventId?.trim()) {
      throw new ValidationError('eventId', eventId, 'Event ID is required');
    }

    console.log(`🔍 Fetching anti-cheat event: ${eventId}`);

    const response = await apiClient.get<ApiResponse<AntiCheatEvent>>(
      `/moderation/anticheat/${eventId}`
    );
 ...
 */
export async function getPlayerAntiCheatStats(
  playerId: string
): Promise<ApiResponse<{
  totalEvents: number;
  ...
  try {
    ...
    >(`/moderation/anticheat/player/${playerId}/stats`);
 ...
 */
function sanitizeString(input: any, fieldName: string, options: {
  maxLength?: number;
  minLength?: number;
  required?: boolean;
  pattern?: RegExp;
  ...
  if (options.required && !str) {
    throw new ValidationError(fieldName, input, `${fieldName} is required`);
  }

  // Check length constraints
  if (options.minLength && str.length < options.minLength) {
    throw new ValidationError(fieldName, input, `${fieldName} must be at least ${options.minLength} characters`);
  }

  if (options.maxLength && str.length > options.maxLength) {
    throw new ValidationError(fieldName, input, `${fieldName} must not exceed ${options.maxLength} characters`);
  }

  // Check pattern
  if (options.pattern && !options.pattern.test(str)) {
    throw new ValidationError(fieldName, input, `${fieldName} format is invalid`);
  }

  // Check allowed characters (basic XSS prevention)
  if (options.allowedChars && !options.allowedChars.test(str)) {
    throw new ValidationError(fieldName, input, `${fieldName} contains invalid characters`);
  }
 ...
 */
function sanitizeNumber(input: any, fieldName: string, options: {
  required?: boolean;
  min?: number;
  max?: number;
  integer?: boolean;
  ...
  if (isNaN(num)) {
    throw new ValidationError(fieldName, input, `${fieldName} must be a valid number`);
  }
  ...
  if (options.integer && !Number.isInteger(num)) {
    throw new ValidationError(fieldName, input, `${fieldName} must be an integer`);
  }

  if (options.min !== undefined && num < options.min) {
    throw new ValidationError(fieldName, input, `${fieldName} must be at least ${options.min}`);
  }

  if (options.max !== undefined && num > options.max) {
    throw new ValidationError(fieldName, input, `${fieldName} must not exceed ${options.max}`);
  }
 ...
 */
function sanitizeArray(input: any, fieldName: string, options: {
  required?: boolean;
  maxLength?: number;
  itemValidator?: (item: any, index: number) => void;
} = {}): any[] {
  if (!Array.isArray(input)) {
    throw new ValidationError(fieldName, input, `${fieldName} must be an array`);
  }

  if (options.required && input.length === 0) {
    throw new ValidationError(fieldName, input, `${fieldName} cannot be empty`);
  }

  if (options.maxLength && input.length > options.maxLength) {
    throw new ValidationError(fieldName, input, `${fieldName} cannot exceed ${options.maxLength} items`);
  }
  ...
  if (options.itemValidator) {
    input.forEach((item, index) => {
      try {
        ...
        throw new ValidationError(`${fieldName}[${index}]`, item,
          `Invalid item at index ${index}: ${error instanceof Error ? error.message : 'Validation failed'}`);
      }
    });
  }
 ...
 */
export function validateAntiCheatEvent(event: any): AntiCheatEvent {
  if (!event || typeof event !== 'object') {
    throw new ValidationError('event', event, 'Event must be a valid object');
  }
  ...
  if (isNaN(parsedTimestamp)) {
    throw new ValidationError('timestamp', timestamp, 'Invalid timestamp format');
  }
  ...
  if (parsedTimestamp > now + fiveMinutes) {
    throw new ValidationError('timestamp', timestamp, 'Timestamp cannot be in the future');
  }

  // Check timestamp is not too old (24 hours)
  const twentyFourHours = 24 * 60 * 60 * 1000;
  if (parsedTimestamp < now - twentyFourHours) {
    throw new ValidationError('timestamp', timestamp, 'Timestamp is too old (over 24 hours)');
  }
 ...
 */
export async function batchSubmitAntiCheatEvents(
  ...
  try {
    if (!Array.isArray(events)) {
      throw new ValidationError('events', events, 'Events must be an array');
    }

    if (events.length === 0) {
      throw new ValidationError('events', events, 'At least one event is required');
    }
    ...
    >('/moderation/anticheat/batch', { events });
